...
async function getFromContentTypes(ext, inline) {
	try {
		const names = await fsp.readdir("modules/shop/front_end/content_types/");
		let ret = "";
		for (const name of names) {
			if(name.substr(0,1) === "."){continue;}
			const prefix = "modules/shop/front_end/content_types/" + name + "/";
...
async function getFromThemes(ext, inline) {
	try {
		const names = await fsp.readdir("modules/shop/front_end/themes/");
		let ret = "";
		for (const name of names) {
			if(name.substr(0,1) === "."){continue;}
			const prefix = "modules/shop/front_end/themes/" + name + "/";
			ret += await getFromPath(ext, prefix, inline);
		}
		return ret;
	}
