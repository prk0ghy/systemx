...
export const reqLogin = async context => {
	...
	context.body = await template.renderPage("login", values, false, context);
};
...
const requestHomePage = async context => {
	switch (configuration.get("startpage")) {
	case "shop":
		return await shop.requestGetShop(context);
	default:
	case "login":
		return await reqLogin(context);
	}
};
export const reqFilter = async (context, next) => {
	if ((context.request.path === "/") || (context.request.path === "")) {
		return requestHomePage(context, next);
	}
	else if (await frontEndSession.checkPassword(context)) {
...
export const reqRelaxedFilter = async (context, next) => {
	if (frontEndSession.check(context) && await pwreset.changeCheck(context)) {
		return pwreset.changePage(context);
	}
	return next(context);
};
...
const reqGetImpressum = async context => {
	const values = {};
	values.title = "Impressum";
	context.body = await template.renderPage("impressum", values, false, context);
};
const reqGetDatenschutz = async context => {
	const values = {};
	values.title = "Datenschutzerklärung";
	context.body = await template.renderPage("datenschutz", values, false, context);
};
