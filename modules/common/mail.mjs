...
export const sendRaw = ({from = null, to, subject, text, html}) => transporter.sendMail({
	from,
	to,
	subject,
	text,
...
export const sendMail = async ({to, templateFile, values}) => {
	if (!to) {
		return false;
	}

	const from = process.env.SYSTEMX_MAIL_FROM;
	const html = await template(templateFile + '.html', values);
	if (!html) {
		return false;
	}

	const text = await template(templateFile + '.txt', values);
	if (!text) {
		return false;
	}
	...
	if (!matches || (matches.length < 2)) {
		console.error('Couldn\'t find a valid <title> in ' + templateFile);
		return false;
	}

	const subject = String(matches[1]);
	return sendRaw({from, to, subject, html, text});
};
